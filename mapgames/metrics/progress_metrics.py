...
class ProgressMetrics:
    ...
    def __init__(self, label, save_path, file_name):
        ...
        self.file.write(
            f"n_eval,{prefixe}coverage,{prefixe}min_fitness,{prefixe}max_fitness,{prefixe}sum_fitness,{prefixe}mean_fitness,{prefixe}median_fitness,{prefixe}max_actor_id\n"
        )
    ...
    def update(self, n_evals, archive):
        ...
        fit_list = np.array([x.fitness for x in archive.get_all_individuals()])

        if len(archive) > 0:
            self.max_actor_id = max(archive.get_all_individuals(), key=lambda x: x.fitness).id
            self.coverage = len(archive)
